...
function LoadMesh(gl, objectRep) {
	...
	for (var i = 1; i <= nface; i++) {
		...
		objectRep.renderProperties.positions.push(x[i0], y[i0], z[i0], x[i1], y[i1], z[i1], x[i2], y[i2], z[i2]);
		i0 = mesh.data.facetnorms[i].i;
		i1 = mesh.data.facetnorms[i].j;
		i2 = mesh.data.facetnorms[i].k;
		objectRep.renderProperties.normals.push(i0, i1, i2, i0, i1, i2, i0, i1, i2);
		i0 = mesh.data.face[i].textCoordsIndex[0] - 1;
		i1 = mesh.data.face[i].textCoordsIndex[1] - 1;
		i2 = mesh.data.face[i].textCoordsIndex[2] - 1;
		objectRep.renderProperties.texcoords.push(xt[i0], yt[i0], xt[i1], yt[i1], xt[i2], yt[i2]);
	}
	objectRep.renderProperties.numVertices = 3 * nface;

	if (mesh.fileMTL == null) {
		objectRep.renderProperties.ambient = mesh.materials[0].parameter.get("Ka");
		objectRep.renderProperties.diffuse = mesh.materials[0].parameter.get("Kd");
		objectRep.renderProperties.specular = mesh.materials[0].parameter.get("Ks");
		objectRep.renderProperties.emissive = mesh.materials[0].parameter.get("Ke");
		objectRep.renderProperties.shininess = mesh.materials[0].parameter.get("Ns");
		objectRep.renderProperties.opacity = mesh.materials[0].parameter.get("Ni");
	}
	else { //TODO: Qui si lavorerebbe per il multi mesh
		objectRep.renderProperties.ambient = mesh.materials[1].parameter.get("Ka");
		objectRep.renderProperties.diffuse = mesh.materials[1].parameter.get("Kd");
		objectRep.renderProperties.specular = mesh.materials[1].parameter.get("Ks");
		objectRep.renderProperties.emissive = mesh.materials[1].parameter.get("Ke");
		objectRep.renderProperties.shininess = mesh.materials[1].parameter.get("Ns");
		objectRep.renderProperties.opacity = mesh.materials[1].parameter.get("Ni");
	}
